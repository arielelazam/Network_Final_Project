...
def connect_to_app(app_ip):
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as tcp_sock: # ניצור סוקט ממשפחת IPv4 ומסוג tcp
            ...
            movies_index = 1
            for movie_number, movie_info in list_response.items():
            ...
            total_number_of_segments = select_response["segments"] # נשמור את מספר הסגמנטים שמרכיבים את הסרט שבחרנו
            quantiles = select_response["quantiles"] # נשמור את אפשרויות האיכויות שסרט מציע

            print(f"The movie contains {total_number_of_segments} segments and the available qualities are: {', '.join(quantiles)}")
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_sock: # נגדיר סוקט ממשפחת IPv4 ומסוג ucp
        ...
        for segment in range(total_number_of_segments):
            print(f"Segment number: {segment + 1}/{total_number_of_segments}. Requested Quality: {current_quality}\n")
            ...
            try:
                while True:
                    data, server_addr = udp_sock.recvfrom(6500) # נשמור את המידע שהתקבל ואת כתובת השרת ששלח לנו את המידע
                    packet = decode_json(data) # נבצע פענוח והמרה ל-JSON של החבילה שהתקבלה
                    seq = packet["seq"] # נשמור את מספר הרצף שהתקבל
